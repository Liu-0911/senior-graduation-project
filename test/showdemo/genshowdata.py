...
def sklearn_pca(legal_feature,legal_feature_70,illegal_feature):        #计算PCA模型
    ...
    print(f"保留95%方差的主成分个数为{resu_ill.shape[1]+1}")
...
# print(f'指尖触摸检测和心脏周期分割用时{str(end - start)}s')
...
list_w_channel_red, list_w_channel_green, list_w_channel_blue = genMaskandW(list_red,
                                                                            ...
                                                                            15,6,0.5)
# end = time.perf_counter()
# print(f'动态像素选择和心脏波行推导用时{str(end - start)}s')
...
# end = time.perf_counter()
# print(f'数据校准和归一化用时{str(end - start)}s')
print("-------------------数据校准和归一化完成-------------------")
...
# end = time.perf_counter()
# print(f'基准特征提取用时{str(end - start)}s')
print("-------------------基准特征提取完成-------------------")
...
# np.save(f"../{foldername}/feature_70_done.npy",feature)
# print(feature)
# cap.release()
# print("-------------------非基准特征提取完成-------------------")
...
#非基准特征提取
# start = time.perf_counter()
filtedData_red_h1 = ButterFiltHigh(filtedData_red, 8,4, 30)
filtedData_green_h1 = ButterFiltHigh(filtedData_green, 8,4, 30)
filtedData_blue_h1 = ButterFiltHigh(filtedData_blue, 8,4, 30)
filtedData_red_h2 = ButterFiltHigh(filtedData_red, 8,5, 30)
filtedData_green_h2 = ButterFiltHigh(filtedData_green, 8,5, 30)
filtedData_blue_h2 = ButterFiltHigh(filtedData_blue, 8,5, 30)
minmaxNormalData_red_h1,minmaxNormalTime_red_h1 = MinMaxNormalization(list_valley,filtedData_red_h1)
minmaxNormalData_green_h1,minmaxNormalTime_green_h1 = MinMaxNormalization(list_valley,filtedData_green_h1)
minmaxNormalData_blue_h1,minmaxNormalTime_blue_h1 = MinMaxNormalization(list_valley,filtedData_blue_h1)
minmaxNormalData_red_h2,minmaxNormalTime_red_h2 = MinMaxNormalization(list_valley,filtedData_red_h2)
...
cap.release()
# end = time.perf_counter()
# print(f'非基准特征提取用时{str(end - start)}s')
print("-------------------非基准特征提取完成-------------------")
...
#用户验证模型建立
# start = time.perf_counter()
legal_feature_70 = np.load(f'../{foldername}/feature_70_done.npy')
...
eit,YouDenJ = RecuGenYouDenJ(list_dist_legal_3,list_dist_illegal)
# end = time.perf_counter()
# print(f'用户验证模型建立用时{str(end - start)}s')
print('-------------------用户验证模型建立完成-------------------')

#用户档案匹配
# start = time.perf_counter()
legal_feature = np.load(f'../{foldername}/feature_70_done.npy')
unver_legal = np.load(f'../{foldername}/legal/feature.npy')
unver_illegal = np.load(f"../{foldername}/illegal/feature.npy")
# resu_ver_ill = verify(list_dist_illegal,eit)
# resu_ver_legal = verify(list_dist_legal_3,eit)
# print(resu_ver_ill)
# print(resu_ver_legal)
# print("下面进行合法用户验证(使用3个心脏周期)")
# verify_ori(legal_feature[0:3,:],legal_feature,eit)
# print("下面进行非法用户验证(使用3个心脏周期)")
# verify_ori(illegal_feature[0:3,:],legal_feature,eit)
print("下面进行合法用户验证(使用3个心脏周期)")
verify_ori(unver_legal[0:3,:],legal_feature,eit)
print("下面进行非法用户验证(使用3个心脏周期)")
verify_ori(unver_illegal[0:3,:],legal_feature,eit)
# end = time.perf_counter()
# print(f'用户档案匹配用时{str(end - start)}s')
